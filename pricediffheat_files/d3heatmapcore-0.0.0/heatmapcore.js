function heatmap(selector, data, options) {
  ...
  function colormap(svg, data, width, height) {
    ...
    var tip = d3.tip()
        ...
        .html(function(d, i) {
          return "<table>" + 
            "<tr><th align=\"right\">Origin</th><td>" + htmlEscape(data.rows[d.row]) + "</td></tr>" +
            "<tr><th align=\"right\">Destination</th><td>" + htmlEscape(data.cols[d.col]) + "</td></tr>" +
            "<tr><th align=\"right\">Change in fare</th><td>" + htmlEscape(d.label) + "$"+ "</td></tr>" 
            "</table>";
        })
        .direction("se")
